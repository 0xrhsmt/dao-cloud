...
async function main() {
  ...
  const daoCloud = await upgrades.upgradeProxy(proxy, DaoCloud, {
    kind: "uups",
  });
  daoCloud.deployed();
  assert(daoCloud.address === proxy, "proxy address changed");

  // Check new implementation
  const impl2 = await upgrades.erc1967.getImplementationAddress(daoCloud.address);
